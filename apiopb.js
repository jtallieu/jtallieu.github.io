...
/*
 * New EEN.API Object that contains a helper interface for accessing the API.
 * All interface calls return a promise and uses the new EEN.Ajax interface.
 * An EEN.Ajax.Pool is a first parameter to all interface functions. If an
 * EEN.Ajax.Pool is not defind then the default pool will be used.
 *
 * The interface handles interpretting the xhr status code and will resolve
 * if the expected xhr status code exists or will reject. Both branches will
 * resolve/reject with a API.Response object instance.
 */
var EEN = (window.EEN) || {};
EEN.API = (function() {
    var API = {};
    ...
    /*
     * Number of maximum default request for the internal
     * default pool
     */
    API._MAX_POOL_REQUESTS = 5;

    /*
     * Default pool used if non is provided in the functional API
     * interfaces
     */
    var _pool = new EEN.Ajax.Pool(API._MAX_POOL_REQUESTS);

    /*
     * Define the API return object that is resolved after every
     * xhr request.
     */
    API.Response = function(xhr, body) {
        var self = this;
        self.xhr = xhr;    //Original XHR result object
        self.body = body;   //Body in format expected by API call
    };

    /*
     * Function calls /g/layout to get layout information
     *
     * @param pool The pool to use
     * @param id The layout id to fetch
     */
    API.getLayout = function(pool, id) {
        pool = pool || _pool;

        //Create a request
        var req = EEN.Ajax.newGetRequest('/g/layout?id=' + id);
        return pool.push(req).then(function(xhr) {
            var resp = new API.Response(xhr, xhr.response);

            //Check if we have a non 200
            if (xhr.status !== 200) {
                resp.body = xhr.response;
    ...
    /*
     * Function updates an existing layout with a body.
     *
     * @param pool The pool to use for the API request
     * @param layout The layout javascript object to save
     * Format:
     * {
     *   panes: [],
     *   settings: {
     *     camera_name: <bool>,
     *     camera_border: <bool>,
     *     camera_aspect_ratio: <string decimal>,
     *     camera_row_limit: <int>,
     *     custom_id: <string>
     *   }
     * }
     *
     */
    API.updateLayout = function(pool, layout_id, layout) {
        pool = pool || _pool;
        var body = {};
        body.id = layout_id;
        body.json = JSON.stringify(layout);
        var req = EEN.Ajax.newPostRequest('/g/layout', { request_type: 'application/json', body: JSON.stringify(body) })
        return pool.push(req).then(function(xhr) {
            var resp = new API.Response(xhr, xhr.response);
